...
```{r}
lm1 <- lm(closed_chest_cvp ~ qrs_rel_index * insp_rel_index + time_s, data = pt_sliced)

BIC(cvp_gam, lm1)
```

```{r}
nlm1 <- lm(closed_chest_cvp ~ poly(qrs_rel_index, 12) + poly(insp_rel_index, 8) + qrs_rel_index:insp_rel_index + poly(time_s, 3), data = pt_sliced)

BIC(nlm1, cvp_gam, lm1)
```
