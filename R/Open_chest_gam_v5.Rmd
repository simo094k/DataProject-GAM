...
stacked_plot <- function(io, ic, qo, qc){
...
server <- function(input, output) {
  ...
  # For snap plots
  preds <- reactive({create_patient_preds(pt_data_open(), pt_data_closed())})
  snapplot <- reactive({create_viz(pred_both = preds(),
                                   patient_number = toString(strtoi(str_sub(input$patient,-2))))})


  # For stacked plot
  stackedp <- reactive({stacked_plot(give_it_center(gratia::evaluate_smooth(pt_data_open(),'s(insp_rel_index)'),input$center_open),
  ...
  contour_samlet <- reactive({create_contour_viz(data_contour_patient(), toString(strtoi(str_sub(input$patient,-2))))})
