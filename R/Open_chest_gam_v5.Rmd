...
create_contour_patient <- function(open_data, closed_data){
  gather_in_one <- rbind(open_data, closed_data)
  gather_in_one$indicator <- cbind(c(rep("open_chest", nrow(open_data)),
                   rep("closed_chest", nrow(closed_data))))
  return(gather_in_one)
}

create_contour_viz <- function(gather_in_one, patient_number){
...
ui <- fluidPage(theme = shinytheme("sandstone"),
  navbarPage("GAM on heart-lung interaction during thoractomy", # Page header
    ...
    tabPanel("QRS og INSP",
             ...
             mainPanel( plotOutput(outputId = "insp_plot"), tableOutput('table')),
...
server <- function(input, output) {
  ...
  # For contour plot
  #kontur_open <- reactive({gratia::draw(pt_data_open(), select =3)})
  
  #kontur_closed <- reactive({gratia::draw(pt_data_closed(), select =3)})
  ...
  contour_samlet <- reactive({create_contour_viz(data_contour_patient(), input$patient)})
...
#    testp <- qplot(rnorm(500),fill=I("red"),binwidth=0.2,main="plotgraph1")
    plotmatrix <- rbind(c(1,2),
                        c(3,3),
                        c(4,4))
    plotlist <- list(stackedplotINSP(),stackedplotQRS(),snapplot(), contour_samlet())
...
#    wtlist <- wtlist[to_delete]
  ...
  first_table <- reactive({subset(table_values, Model %in% c(paste0(input$patient, "OPEN.rds", sep=""),paste0(input$patient, "CLOSED.rds", sep="")), select = c("Model", "CV_MAE", "Model_MAE")) })
  ...
  diff_time <- reactive({ c(max(data_time_closed()$time_s) - min(data_time_closed()$time_s), max(data_time_open()$time_s) - min(data_time_open()$time_s)) })
  
  #final_table <- reactive({cbind(first_table(), start_time(), end_time(), diff_time())})
  
  output$table <- renderTable({cbind(first_table(), start_time(), end_time(), diff_time())})
