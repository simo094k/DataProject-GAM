  ...
  navbarPage("GAM on heart-lung interaction during thoractomy", # Page header
    ...
    tabPanel("Main page",
        ...
        sidebarPanel(h3("Inputs"),
                    ...
                    tags$style(type="text/css", "#table tr:nth-child(1), tr:nth-child(5), tr:nth-child(10)  { border-bottom: solid 2px black;}"),
                    htmlOutput("text3")),
...
server <- function(input, output) {
  ...
  model_table <- reactive({subset(table_values, Model %in% c(paste0(input$patient, "OPEN.rds", sep=""),
                                                             paste0(input$patient, "CLOSED.rds", sep="")),
                                  select = c("Model")) })
  
  cv_mae_table <- reactive({subset(table_values, Model %in% c(paste0(input$patient, "OPEN.rds", sep=""),
                                                             paste0(input$patient, "CLOSED.rds", sep="")),
                                  select = c("CV_MAE")) })
  
  mae_table <- reactive({subset(mae_values, Model %in% c(paste0(input$patient, "OPEN.rds", sep=""),
                                                             paste0(input$patient, "CLOSED.rds", sep="")),
                                  select = c("Actual Model MAE")) })
  
 mae_table2 <- reactive({subset(mae_values, Model %in% c(paste0(input$patient, "OPEN.rds", sep=""),
                                                             paste0(input$patient, "CLOSED.rds", sep="")),
                                  select = c("LM MAE","Poly1 MAE","Poly2 MAE")) })
  
  bic_table <- reactive({subset(bic_values, Model %in% c(paste0(input$patient, "OPEN.rds", sep=""),
                                                             paste0(input$patient, "CLOSED.rds", sep="")),
                                  select = c("Actual Model BIC","LM BIC","Poly1 BIC","Poly2 BIC")) })
  
  
  intercept <- reactive({c(pt_data_open()[["coefficients"]][1],pt_data_closed()[["coefficients"]][1] )  })
  ...
  output$table <- renderTable({t(cbind(model_table(), start_time(), end_time(), diff_time(), intercept(), mae_table(), cv_mae_table(), mae_table2(), bic_table()))}, rownames=TRUE, colnames=FALSE)
  
  output$text3 <- renderUI({
                str1 <- "LM: linear Regression Model "
                str11 <- "CVP = &beta;&#95;0 (qrs_rel_index &times; insp_rel_index) + &beta;&#95;1 qrs_rel_index + &beta;&#95;2 insp_rel_index + &beta;&#95;3 time + &epsilon;."
                str111 <- ""
                str2 <- "Poly1: Polynomial Regression Model 1"
                str22 <- "CVP = &beta;&#95;0 qrs_rel_index^12 + &beta;&#95;1 insp_rel_index^8 + &beta;&#95;2 time^3 + &epsilon;."
                str222 <- ""
                str3 <- "Poly 2: Polynomial Regression Model 2"
                str33 <- "CVP = &beta;&#95;0 qrs_rel_index^23 + &beta;&#95;1 insp_rel_index^23 + &beta;&#95;2 time^23 + &epsilon;."
                str333 <- ""
