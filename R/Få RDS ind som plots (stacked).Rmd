...
server <- function(input, output) {
 ...
 # open_times <- round(c(min(data_time_open()$time_s) , max(data_time_open()$time_s), max(data_time_open()$time_s) - min(data_time_open()$time_s)),2)
  ...
  output$maksimum_closed <- renderText({ as.character(closed_times()[1]) })
...
```{r}
filepath <- "../GAM_RDS/pt37OPEN.rds"
full_dataload <- readRDS(filepath, refhook = NULL)

filepath <- "../GAM_RDS/pt37CLOSED.rds"
...
```{r}
filepath <- "../GAM_RDS/pt10OPEN.rds"
full_dataload <- readRDS(filepath, refhook = NULL)

filepath <- "../GAM_RDS/pt10CLOSED.rds"
full_dataloadclosed <- readRDS(filepath, refhook = NULL)
...
## highest point same place
```{r samme sted}
data_insp_open <- gratia::evaluate_smooth(full_dataload, 's(insp_rel_index)')
data_insp_closed <- gratia::evaluate_smooth(full_dataloadclosed, 's(insp_rel_index)')


if (which.max(data_insp_open$est) > which.max(data_insp_closed$est) ){
  biggest <- which.max(data_insp_open$est)
  smallest <- which.max(data_insp_closed$est)
  
  value_to_add <- data_insp_open$insp_rel_index[biggest] - data_insp_closed$insp_rel_index[smallest]
  
  data_insp_closed$insp_rel_index <- data_insp_closed$insp_rel_index + value_to_add

  for (index in 1:length(data_insp_closed$insp_rel_index)) {
    if (data_insp_closed$insp_rel_index[index] > 1 ){
      data_insp_closed$insp_rel_index[index] <- data_insp_closed$insp_rel_index[index] -1
    }
  }
} else{
  biggest <- which.max(data_insp_closed$est)
  smallest <- which.max(data_insp_open$est)
  
  value_to_add <- data_insp_closed$insp_rel_index[biggest] - data_insp_open$insp_rel_index[smallest]
  
  data_insp_open$insp_rel_index <- data_insp_open$insp_rel_index + value_to_add

  for (index in 1:length(data_insp_open$insp_rel_index)) {
    if (data_insp_open$insp_rel_index[index] > 1 ){
      data_insp_open$insp_rel_index[index] <- data_insp_open$insp_rel_index[index] -1
    }
  }
}
...
## highest point at center 0.5 - til andreas at bruge
```{r fælles på midten}
data_insp_open <- gratia::evaluate_smooth(full_dataload, 's(insp_rel_index)')
data_insp_closed <- gratia::evaluate_smooth(full_dataloadclosed, 's(insp_rel_index)')

if (which.max(data_insp_open$est) > 50 ){
  biggest <- which.max(data_insp_open$est)
  
  value_to_subtract <- data_insp_open$insp_rel_index[biggest] - data_insp_open$insp_rel_index[50]
  
  data_insp_open$insp_rel_index <- data_insp_open$insp_rel_index - value_to_subtract

  for (index in 1:length(data_insp_open$insp_rel_index)) {
    if (data_insp_open$insp_rel_index[index] < 0 ){
      data_insp_open$insp_rel_index[index] <- data_insp_open$insp_rel_index[index] +1
    }
  }
} else{
  ...
  value_to_add <- data_insp_open$insp_rel_index[50] - data_insp_open$insp_rel_index[smallest]
...
if (which.max(data_insp_closed$est) > 50 ){
  biggest <- which.max(data_insp_closed$est)
  
  value_to_subtract <- data_insp_closed$insp_rel_index[biggest] - data_insp_closed$insp_rel_index[50]
  
  data_insp_closed$insp_rel_index <- data_insp_closed$insp_rel_index - value_to_subtract

  for (index in 1:length(data_insp_closed$insp_rel_index)) {
    if (data_insp_closed$insp_rel_index[index] < 0 ){
      data_insp_closed$insp_rel_index[index] <- data_insp_closed$insp_rel_index[index] +1
    }
  }
} else{
  ...
  value_to_add <- data_insp_closed$insp_rel_index[50] - data_insp_closed$insp_rel_index[smallest]
...
https://stackoverflow.com/questions/27827962/r-shiny-bi-directional-reactive-widgets
...
```{r}
aa<- give_it_center(data_insp_open)
bb <- give_it_center(data_insp_closed)


ggplot() +
  geom_line(data=aa, aes(insp_rel_index,est, colour="open")) +
  geom_line(data=bb, aes(insp_rel_index,est, colour="closed")) +
  scale_color_manual(values = c(
    'open' = 'darkblue',
    'closed' = 'red')) +
  labs(color = 'Status')
...
closed_times <- round(c(min(data_time_closed$time_s) , max(data_time_closed$time_s), max(data_time_closed$time_s) - min(data_time_closed$time_s)),2)
