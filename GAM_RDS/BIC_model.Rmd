...
dat_frame <- data.frame(matrix(ncol = 5, nrow = 0))
x <- c("Model", "Actual Model BIC", "LM BIC", "Poly1 BIC", "Poly2 BIC")
...
for (name in df){
  ...
  if(!is.null(gam_obj$model$cvp)  & !is.null(gam_obj$model$qrs_rel_index) & !is.null(gam_obj$model$insp_rel_index) & !is.null(gam_obj$model$time_s)) {
   ...
   # spline <- lm(gam_obj$model$cvp ~ bs(gam_obj$model$qrs_rel_index * gam_obj$model$insp_rel_index + gam_obj$model$time_s))
    ...
    bic_model <- BIC(gam_obj, lm1, nlm1, nlm2)
    ...
    dat_frame[qwerty, 3] <- bic_model[2, 2] #Lm_model
    dat_frame[qwerty, 4] <- bic_model[3, 2] #poly_12
    dat_frame[qwerty, 5] <- bic_model[4, 2] #poly_23
    qwerty <- qwerty + 1
  }
...
#resp_data <- readRDS("../Datasets/thoracotomy_waveforms/comb_ventilation_data.RDS")
dat_frame <- data.frame(matrix(ncol = 5, nrow = 0))
x <- c("Model", "Actual Model MAE", "LM MAE", "Poly1 MAE", "Poly2 MAE")
colnames(dat_frame) <- x
qwerty <- 1
df <- list.files(pattern = ".rds")
...
for (name in df){
  ...
  #  bic_model <- BIC(gam_obj, lm1, nlm1, nlm2)
    ...
    dat_frame[qwerty, 3] <- mae_lm #MAE lm
    dat_frame[qwerty, 4] <- mae_poly_12 #poly_12_model
    dat_frame[qwerty, 5] <- mae_poly_23 #poly_12
    qwerty <- qwerty + 1
  }
